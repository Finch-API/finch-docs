...
---------|----------|---------
 ...
 `payroll_provider` | false | An optional parameter that allows users to bypass the provider selection screen. Compatible payrolls are: `adp_workforce_now`, `bamboo_hr`, `gusto`, `insperity`, `justworks`, `paylocity`, `quickbooks`, `rippling`, `adp_run`, `square_payroll`, `trinet`, and `zenefits`.
